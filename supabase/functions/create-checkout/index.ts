...
// No more hardcoded map — we fetch stripe_price_id from the plans table
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: planData, error: planError } = await dbClient
      ...
      .eq("id", planId)
      .single();
    if (planError || !planData?.stripe_price_id) throw new Error("Plano não encontrado ou não configurado no Stripe");
